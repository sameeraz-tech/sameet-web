...
sameet Signaling Server
Copyright (C) 2021 SAM <smt.web>
...
let API_KEY_SECRET = process.env.API_KEY_SECRET || "sam_chat";
...
app.post(["/api/v1/meeting"], (req, res) => {
  ...
  if (authorization != API_KEY_SECRET) {
    logme("sameet get meeting - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
    return res.status(403).json({ error: "Unauthorized!" });
  }
  // setup sameet meeting URL
  ...
  logme("sameet get meeting - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
